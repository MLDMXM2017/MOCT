...
class tree_stump(object):
    ...
    def _predict(self, node, row, c_test, c_train):

        label = []
        error = None
        min_dis = np.inf
        for i, qua in enumerate(c_train):
...
class MOCT(BaseEstimator):

    def __init__(self, *, max_depth=10, min_samples_leaf=0.1, criterion="gini", method="SMOTE", n_jobs=4):
    ...
    def ova_change(self, data):
        ...
        evaluate_0 = list(Parallel(n_jobs=self.n_jobs)(delayed(self.classification_node)(data_0, seed) for seed in range(self.n_feature)))
        evaluate_1 = list(Parallel(n_jobs=self.n_jobs)(delayed(self.classification_node)(data_1, seed) for seed in range(self.n_feature)))
        evaluate_2 = list(Parallel(n_jobs=self.n_jobs)(delayed(self.classification_node)(data_2, seed) for seed in range(self.n_feature)))
    ...
    def get_split(self, data, clf_0, clf_1, clf_2):
        ...
        return {'evaluate': [clf_0, clf_1, clf_2], 'groups': groups, "distribute": [len(i) for i in groups], "probability":[[row[-1] for row in group] for group in groups]}
    ...
    def _predict_proba(self, node, row):
        ...
        if int(label_0[0]) == 0 and int(label_1[0]) != 1 and int(label_2[0]) != 2:
            ...
            return node["probability"][1], 1
        elif int(label_0[0]) != 0 and int(label_1[0]) != 1 and int(label_2[0]) == 2:
            return node["probability"][2], 2
        else:
            if 'next' in node:
                return self._predict_proba(node['next'], row)
            else:
                return node["probability"][3], node["value"]
    ...
    def predict_proba(self, x):
        ...
        for num, row in enumerate(x):
            y_p = [0, 0, 0]
            y_proba, y_pred = self._predict_proba(self.tree, row)
            if len(y_proba) == 0:
                y_p[y_pred] = 1
            else:
                ...
                for i in y_proba:
                    if i == 0:
                        ...
                        num_2 = num_2 + 1
                y_p[0] = num_0/len(y_proba)
                y_p[1] = num_1/len(y_proba)
                y_p[2] = num_2/len(y_proba)
            df_empty.append(y_p)
